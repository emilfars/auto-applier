...
**Current phase:** M3 — ingestion + feed (in progress). Ingestion domain + scraper runner landed: normalizer + Tier-restricted registry (AC-SCR-2/2b), plus a per-source circuit-breaker Runner and an HTML board scraper validated against a fixture server (AC-SCR-1/1b). Next: dedup persistence (SCR-3), staleness (SCR-4), feed API (FEED-1..3).
...
| M3 Ingestion + feed | SCR-1..4, FEED-1..3, seed ≥5k listings | 🟡 | Ingestion domain + runner: normalizer (AC-SCR-2) + Tier-restricted registry (AC-SCR-2b); per-source circuit-breaker `Runner` persisting normalized jobs (AC-SCR-1) with one failing source isolated so the feed stays non-empty (AC-SCR-1b); Tier-2 `HTMLSource` validated against a fixture HTML server. Next: dedup persistence (SCR-3), staleness marking (SCR-4), feed API (FEED-1..3), seed ≥5k. |
...
## 6. Now / Next / Blocked
- **Now:** M3 scraper runner landed (`ingest.Runner`): iterates registered sources with a **per-source circuit breaker** (opens after N consecutive failures, half-opens after a cooldown, closes on success), normalizes each RawJob and upserts by dedup key. A failing/open source is isolated per-source so the run never aborts and the feed stays non-empty (AC-SCR-1b). A Tier-2 `HTMLSource` (config-driven `CardSelectors`, x/net/html) was validated end-to-end against a fixture HTML server, parsing salary/employment/remote and resolving relative + absolute listing URLs (AC-SCR-1). verify.sh green (8/8).
- **Next:** Dedup persistence across a real store (SCR-3), staleness marking within the 48h window with clock injection (SCR-4), then the feed API — paginated stated-only cards (FEED-1), filters (FEED-2), free-text search (FEED-3). Job-queue choice (#4) still deferred; the runner is currently invoked directly (in-process).
