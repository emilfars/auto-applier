...
**Current phase:** M2 — CV & profile **complete**. CV upload, hosted-API parsing, profile edit, and the confirm-before-apply gate all landed (AC-CV-1..5). Next milestone: M3 (ingestion + feed).
...
## 6. Now / Next / Blocked
- **Now:** M2 complete. CV parsing (AC-CV-2) landed via a hosted resume-parse API behind a vendor-agnostic `Parser` interface: `POST /cv/{id}/parse` reads the stored (encrypted) CV, calls the API, maps + normalizes the response, and writes contact/education/work/skills into the profile — leaving it **unconfirmed** so the user reviews before arming. Deterministic id+en fixtures verify ≥90% field accuracy (100%). Parser degrades to 503 when `CV_PARSER_URL` is unset. verify.sh green (8/8).
- **Next:** M3 — ingestion + feed (SCR-1..4 Tier 1–2 scrapers, FEED-1..3 normalized filterable feed with labeled stated-only pay, seed ≥5k listings).
