...
**Current phase:** M3 — ingestion + feed **complete for MVP scope**. Scrapers (SCR-1..4), feed API (FEED-1/2/3), a ≥5k Jabodetabek seed job (SEED-1), and the 50k filter p95 <500ms perf gate (FEED-2b) all landed. Only the perf-stage NFRs remain (FEED-1p 4G, NFR-SCALE) plus the deferred pgx-backed job store. Next milestone: M4 (Chrome extension + packages/fill-mappings).
...
## 6. Now / Next / Blocked
- **Now:** M3 seed + perf gate landed. New `backend/internal/seed` package generates deterministic synthetic **Jabodetabek-first, stated-only** listings via combinatorial indexing so every listing has a unique (company, title, city) dedup key — no silent collapse. `seed.Seed(ctx, store, n, seed, baseTime)` targets any `ingest.JobStore` (in-memory today, pgx later) and a runnable `cmd/seed` loads ≥5,000 listings before signup (AC-SEED-1). Added a feed perf test asserting **filter query p95 < 500ms on a 50k seed** — measured ~19ms (AC-FEED-2b). verify.sh green (8/8).
- **Next:** Start **M4** — the Chrome MV3 extension + the shared, versioned `packages/fill-mappings` (autofill only; **never** submit — Prime Directive). Remaining M3 items are perf-stage NFRs (AC-FEED-1p 4G, AC-NFR-SCALE) and the deferred pgx-backed `JobStore`/feed provider; job-queue choice (#4) and real Google OIDC also still deferred.
