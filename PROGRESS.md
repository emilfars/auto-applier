...
**Current phase:** M3 — ingestion + feed (in progress). All four scraper ACs done (SCR-1..4): normalizer, Tier-restricted registry, circuit-breaker runner + HTMLSource, cross-source dedup, and 48h staleness. Next: the feed API (FEED-1..3) then seed ≥5k listings.
...
| M3 Ingestion + feed | SCR-1..4, FEED-1..3, seed ≥5k listings | 🟡 | Scrapers **done** (SCR-1..4): normalizer + Tier-restricted registry (AC-SCR-2/2b), circuit-breaker Runner + HTMLSource vs fixture server (AC-SCR-1/1b), cross-source dedup (AC-SCR-3), 48h staleness with clock injection (AC-SCR-4). Next: feed API — stated-only cards (FEED-1), filters (FEED-2), search (FEED-3), then seed ≥5k. |
...
## 6. Now / Next / Blocked
- **Now:** M3 dedup + staleness landed (SCR-3/4). Cross-source deduplication is proven end-to-end: the same listing from two sources collapses to one canonical job by dedup key (AC-SCR-3). Staleness is a pure, clock-injectable rule (`IsStale`, `StaleAfter=48h`): the store tracks each job's last-seen time, `SweepStale` marks not-re-seen listings stale within the 48h window and excludes them from `Active()` (the feed set), and re-seeing a listing clears the flag (AC-SCR-4). verify.sh green (8/8).
- **Next:** The feed API — paginated **stated-only** cards with no estimated pay field (FEED-1), filters for pay/location/remote/skills/YoE/employment-type/posted-date/source (FEED-2), and free-text search on title + company (FEED-3). Then seed ≥5k listings. Job-queue choice (#4) still deferred; runner invoked in-process for now.
