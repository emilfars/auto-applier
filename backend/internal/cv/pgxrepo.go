...
const cvColumns = `id::text, user_id::text, object_key, filename, content_type, size_bytes, created_at`

func (r *PgxRepo) CreateFile(ctx context.Context, f File) (File, error) {
	var out File
	err := r.db.QueryRow(ctx, `
		INSERT INTO cv_files (user_id, object_key, filename, content_type, size_bytes, encrypted)
		VALUES ($1, $2, $3, $4, $5, true)
		RETURNING `+cvColumns,
		f.UserID, f.ObjectKey, f.Filename, f.ContentType, f.SizeBytes,
	).Scan(&out.ID, &out.UserID, &out.ObjectKey, &out.Filename, &out.ContentType, &out.SizeBytes, &out.CreatedAt)
	if err != nil {
...
func (r *PgxRepo) FileByID(ctx context.Context, id string) (File, error) {
	...
	).Scan(&f.ID, &f.UserID, &f.ObjectKey, &f.Filename, &f.ContentType, &f.SizeBytes, &f.CreatedAt)
...
func (r *PgxRepo) FilesByUser(ctx context.Context, userID string) ([]File, error) {
	rows, err := r.db.Query(ctx,
		`SELECT `+cvColumns+` FROM cv_files WHERE user_id = $1 ORDER BY created_at DESC`, userID)
	if err != nil {
	...
	for rows.Next() {
		var f File
		if err := rows.Scan(&f.ID, &f.UserID, &f.ObjectKey, &f.Filename, &f.ContentType, &f.SizeBytes, &f.CreatedAt); err != nil {
